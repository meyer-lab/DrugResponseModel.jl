...
function ODEjac(p::AbstractVector{T})::Matrix{T} where {T <: Real}
    ...
    A[diagind(A, 0)[1:Int(nG1 / 3)]] .= -(p[1] + p[7])
    A[diagind(A, 0)[Int(nG1 / 3 + 1):Int(2 * nG1 / 3)]] .= -(p[2] + p[8])
    A[diagind(A, 0)[Int(2 * nG1 / 3 + 1):nG1]] .= -(p[3] + p[9])
    A[diagind(A, 0)[(nG1 + 1):Int(nG1 + nG2 / 3)]] .= -(p[4] + p[10])
    A[diagind(A, 0)[Int(nG1 + nG2 / 3 + 1):Int(nG1 + 2 * nG2 / 3)]] .= -(p[5] + p[11])
    A[diagind(A, 0)[Int(nG1 + 2 * nG2 / 3 + 1):nSp]] .= -(p[6] + p[12])

    A[diagind(A, -1)[1:Int(nG1 / 3)]] .= p[1]
    A[diagind(A, -1)[Int(nG1 / 3 + 1):Int(2 * nG1 / 3)]] .= p[2]
    A[diagind(A, -1)[Int(2 * nG1 / 3 + 1):nG1]] .= p[3]
    A[diagind(A, -1)[nG1 + 1:Int(nG1 + nG2 / 3)]] .= p[4]
    A[diagind(A, -1)[Int(nG1 + nG2 / 3 + 1):Int(nG1 + 2 * nG2 / 3)]] .= p[5]
    A[diagind(A, -1)[Int(nG1 + 2 * nG2 / 3 + 1):(nSp - 1)]] .= p[6]
