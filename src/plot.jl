...
function plotperc(g2, name, conc, txt)
    ...
    p4 = plot(time, 100.0 .* g2[:, 1], lw=3, color="red", label = "control ", legend=:best, title=string(name), legendfontsize=5, xlabel = "time [hr]", ylabel = "S/G2 cell cycle (%) ", alpha = 0.7, title=txt)
