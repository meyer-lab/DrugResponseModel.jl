"""Functions to build figures that are NOT used in the paper."""

drugs = ["BEZ235", "Trametinib", "5FU", "AZD5438", "Panobinostat", "MG132", "Everolimus", "JQ1", "Bortezomib", "MK1775", "Cabozantinib"]


""" plot the data that includes AU565 treated with 11 drugs, including BEZ235, 5FU, Trametinib, AZD438, palobinostat, MG132, Everolimjus, JQ1, Bortezomib, MK1775, and cabozaninib. """
...
"""Plot the raw data of G1 ans S-G2 counts normalized to time0."""
function figure1a()
    setGadflyTheme()
    ...
    return draw(SVG("AU565_11drugs_data.svg", 18inch, 20inch), pl)
end
...
function figure2a()
    ...
    pp = []
    pODE = DrugResponseModel.out_ODEparams()
    ...
    pl = plotGrid((6, 4), [pp..., nothing, nothing];)
    return draw(SVG("AU565_11drugs_model.svg", 22inch, 18inch), pl)
end
...
function figure3()
    ...
    pl = plotGrid((2, 4), [ppp...];)
    return draw(SVG("figure3a_PCA.svg", 16inch, 8inch), pl)
end
...
function figure31()
    ...
    pl = plotGrid((2, 4), [ppp...];)
    return draw(SVG("figure3b_PCA.svg", 12inch, 4inch), pl)
end
