...
function temporal_combination(params1, params2, g0, title)
    ...
    g1G = g2G = zeros(96)
    nG1 = Int(floor(params2[6]))
    nG2 = Int(floor(params2[7]))
    nD1 = Int(floor(params2[8]))
    nD2 = Int(floor(params2[9]))
    v = predict2(params2, g1L[end] + g2L[end], t2, nG1, nG2, nD1, nD2,2)
    g1G[1] = sum(v[1:nG1]) + sum(v[(nG1 + nG2 + 1):(nG1 + nG2 + nD1)])
    g2G[1] = sum(v[(nG1 + 1):(nG1 + nG2)]) + sum(v[(nG1 + nG2 + nD1 + 1):(nG1 + nG2 + nD1 + nD2)])
    for j=2:96
        v = predict2(params2, g1G[j-1]+g2G[j-1], t2, nG1, nG2, nD1, nD2,j+1)
        g1G[j] = sum(v[1:nG1]) + sum(v[(nG1 + nG2 + 1):(nG1 + nG2 + nD1)])
        g2G[j] = sum(v[(nG1 + 1):(nG1 + nG2)]) + sum(v[(nG1 + nG2 + nD1 + 1):(nG1 + nG2 + nD1 + nD2)])
    end
    allG1s = vcat(g1L, g1G)
    allG2s = vcat(g2L, g2G)
    allT = vcat(t1, t2)
